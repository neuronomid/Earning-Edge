...
class Activist13DCandidateService:
    ...
    async def _enrich(
        ...
        *,
        ...
        days_to_next_earnings = None if next_earnings is None else (next_earnings - today).days
...
def _event_component_summary(components: tuple[tuple[str, Decimal], ...]) -> str:
    ...
    return ", ".join(
        f"{name}={value.quantize(Decimal('0.01'))}" for name, value in components if name in wanted
    )
...
def _event_component_notes(
    ...
    return tuple(
        f"Activist 13D {name}: {value.quantize(Decimal('0.01'))}" for name, value in components
    )
