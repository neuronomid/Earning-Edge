...
class FeedbackEventRepository(BaseRepository[FeedbackEvent]):
    ...
    async def list_for_recommendation(self, recommendation_id: UUID) -> list[FeedbackEvent]:
        result = await self.session.execute(
            select(FeedbackEvent).where(FeedbackEvent.recommendation_id == recommendation_id)
        )
        return list(result.scalars().all())

    async def delete_for_recommendation_user(self, recommendation_id: UUID, user_id: UUID) -> None:
