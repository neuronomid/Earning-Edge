...
def assess_option_reality(
    ...
    valuation_date = (
        candidate.valuation_date or candidate.market_snapshot.as_of_date or date.today()
    )
...
def _sigma_percent(contract: OptionContractInput, trading_days: int) -> Decimal | None:
    if contract.implied_volatility is None or contract.implied_volatility <= ZERO:
        return None
    return (
        contract.implied_volatility * (Decimal(max(trading_days, 1)) / TRADING_DAYS_PER_YEAR).sqrt()
    )
...
def _reality_flags(
    *,
    ...
    if no_named_catalyst and candidate.strategy_source in {
        ...
        if required_sigma_to_breakeven is not None and required_sigma_to_breakeven > Decimal(
            "1.00"
        ):
